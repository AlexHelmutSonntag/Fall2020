...
class ReadTextFile {
  ...
  public static void writeFileContent(String file, long skipper) {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(input))) {
      ...
      if(linesCount <= skipper) {
        System.out.println("Warning: There are too few lines to skip in the file");
      }
  ...
  public static void main(String[] args) {
    if (args.length < 1 || args.length > 2) {
      System.err.println("Error: Please provide filename (requirement) and a number (requirement)");
      System.exit(1);
    }
    ...
    if (args.length == 2) {
      try {
       skipper = Long.valueOf(args[1]);
        ...
        System.out.println("Warning: If you enter second argument it should only  be a number");
      } 
    }  
